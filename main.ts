...
// knobs
const MAX_QUERY_LENGTH = 40000;
const MAX_RESPONSE_TOKENS = 1500;
const MAX_REQS_PER_MINUTE = 60;
...
serve(async (req: Request): Promise<Response> => {
  ...
  // OpenAI call with basic error handling
  let baseResponse = "No response from OpenAI";
  try {
    const r = await fetch("https://api.openai.com/v1/chat/completions", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${OPENAI_API_KEY}` },
      body: JSON.stringify({ model: "gpt-4o", messages, max_tokens: MAX_RESPONSE_TOKENS }),
    });
    const data = await r.json();
    baseResponse = data?.choices?.[0]?.message?.content || baseResponse;
  } catch {
    return new Response("Error contacting server.", {
      status: 502,
    ...
    try {
      const qt = await fetch(
        `https://${QUALTRICS_DATACENTER}.qualtrics.com/API/v3/surveys/${QUALTRICS_SURVEY_ID}/responses`,
        {
          ...
          body: JSON.stringify({ values: { responseText: result, queryText: query } }),
        },
      );
